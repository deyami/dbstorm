...
import com.reform.dbstorm.xml.DbXmlParser.DbInstanceType;
import com.reform.dbstorm.xml.InstancesXmlParser;
import com.reform.dbstorm.xml.RouteConfig;
import com.reform.dbstorm.xml.StromXmlConfigException;
 ...
 * 命令定义.
 * name:  refreshdb
 * usage: refreshdb -r,--refresh|-a,--add [pathname]
 * options :
 * 			-r,--refresh 从[pathname]中指定的配置文件中读取，并刷新已经定义过的数据库配置
 * 			-a,--add     从[pathname]中指定的配置文件中读取，并添加未定义过的配置
 *
 * @author huaiyu.du@opi-corp.com 2012-1-25 下午12:35:27
 *
 */
public final class UpdateDb extends StormCommand {

    public static final Logger      log = LoggerFactory.getLogger(UpdateDb.class);
    ...
    private class UpdateTask implements StormTask, Comparable<UpdateTask> {
        ...
        private void createDb(DbInstanceConfig newConfig) {
            ...
            List<String> dbList = zkClient.getChildren(CHROOT);
            //预检查,防止产生不完整结果
